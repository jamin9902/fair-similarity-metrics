...
def get_consistency(X, l_pred, gender_idx = 39, man_idx = 33, woman_idx = 38):
    '''
    ...
    woman_logits = l_pred(X_women)
    woman_preds = np.argmax(woman_logits, axis = 1)

    gender_consistency = np.mean([1 if man_preds[i] == woman_preds[i] else 0 for i in range(len(man_preds))])
...
COUNTER_INIT = 0.1
TB_BASE = './tensorboard_hire_explore/'
...
#                        sess.run(reset_full_adv_weights)
            ...
            if (it % out_freq == 0 or it == epoch - 1) and verbose:
                ...
                if plot:
                    ...
                    gender_consistency = get_consistency(X_test, lambda x: l_pred.eval(feed_dict={tf_X: x}))
                    print('Epoch %d gender consistency %g' % (it, gender_consistency))
                        
                    summary = tf.Summary(value=[
                    tf.Summary.Value(tag='gender consistency', simple_value = gender_consistency),
