...
import glob
from hire import preprocess_hire_data, get_metrics
from train_clp_hire import get_consistency, predict_from_checkpoint
...
base_dir = './tensorboard_hire_seeds/'
...
for pack in product(*hypers):
    ...
    for seed_idx, seed in enumerate(seeds):
        X_train, X_test, y_train, y_test, X_gender_train, X_gender_test, y_gender_train, y_gender_test, dataset_orig_train, dataset_orig_test, names_income, names_gender = preprocess_hire_data(seed = seed)
        ...
        with tf.Session() as sess:
            ...
            gender_consistency = get_consistency(X_test, lambda x: predict_from_checkpoint(sess, 'add_' + post_idx, x))
            # print(exp_descriptor)

            acc_temp, bal_acc_temp, gender_gap_rms_temp, gender_max_gap_temp = get_metrics(dataset_orig_test, preds, verbose=False)
            seed_metrics = [acc_temp, bal_acc_temp, gender_consistency,
                            gender_gap_rms_temp, gender_max_gap_temp]
    ...
    print(50*'==')
    print(exp_descriptor)
    print('acc', 'bal_acc', 'gender_consistency',
          'gender_gap_rms', 'gender_max_gap', )
